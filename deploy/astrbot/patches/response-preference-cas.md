...
Iris maintenance still requires exact candidate/scope/source/hash and a separately authorized single-record change. The Iris adapter now reports commit_outcome_unknown when CAS raises, preserves the backup, and uses committed_unverified when confirmed CAS success is followed by a failed readback. Unsupported backends return conditional_write_unavailable. Do not infer that an error response proves zero writes. These integration changes are not deployed or runtime-tested.
