...
_MEME_CONTEXT_MARKER: Final[str] = (
    "[表情包：未获得画面摘要；仅用于表达情绪，不要把它当作普通照片分析]"
)


def _caption_is_meme_marker(caption: str) -> bool:
    """识别视觉模型按约定返回的表情包标记。"""
    normalized = (caption or "").strip().strip("[]【】").strip()
...
def _format_caption(caption: str) -> str:
    ...
    if not text:
        return "[图片]"
    if _caption_is_meme_marker(text):
        detail = re.sub(
            r"^(?:表情包|贴纸|反应图|meme|sticker)\s*[:：\-—]?\s*",
            "",
            text,
            flags=re.IGNORECASE,
        ).strip(" []【】")
        if detail and detail.lower() not in {"表情包", "贴纸", "反应图", "meme", "sticker"}:
            return f"[表情包: {detail[:120]}]"
        return _MEME_CONTEXT_MARKER
    return f"[图片: {text[:120]}]"

def _is_platform_sticker(
    event: AstrMessageEvent,
    component: Image,
    image_index: int,
) -> bool:
    """用 QQ/OneBot 元数据识别贴纸，避免把表情包交给视觉转述。"""

    def is_sticker_value(value: Any) -> bool:
        if value is None:
            return False
        try:
            return int(value) == 1
    ...
    def data_is_sticker(data: Any) -> bool:
        if not isinstance(data, dict):
            return False
        subtype = data.get("sub_type")
        if subtype is None:
            subtype = data.get("subType")
        if is_sticker_value(subtype):
            return True
        summary = str(data.get("summary") or "").strip().lower()
        if "表情" in summary or "emoji" in summary or "sticker" in summary:
            return True
        image_type = str(
            data.get("type")
        ...
        return image_type in {"emoji", "sticker", "face", "meme"}
    ...
    try:
        ...
        raw_message = (
            ...
            else getattr(raw_event, "message", None)
        )
        if isinstance(raw_message, list):
            ...
            if 0 <= image_index < len(image_segments):
                if data_is_sticker(image_segments[image_index].get("data", {})):
                    return True
    except Exception:
        pass

    for attribute in ("subType", "sub_type"):
        if is_sticker_value(getattr(component, attribute, None)):
            return True

    try:
        raw_data = component.toDict()
        if isinstance(raw_data, dict) and data_is_sticker(raw_data.get("data", raw_data)):
            return True
    except Exception:
        pass
        ...
        *,
        ...
        if show_recent_images:
            ...
            for m in image_source:
                ...
                has_caption = bool(re.search(r"\[(?:图片|表情包)\s*[:：]", content))
                if (
                    m.has_gif
                    and not show_recent_gifs
    ...
    async def _lazy_caption_flow(
        ...
        for msg in messages:
            ...
            for img_idx, url in enumerate(msg.image_urls):
                ...
                has_local_copy = bool(
                    msg.image_local_paths
                    and img_idx < len(msg.image_local_paths)
                    and msg.image_local_paths[img_idx]
                    and os.path.exists(msg.image_local_paths[img_idx])
                )
                if is_gif and not self._show_recent_images_allow_gif and not has_local_copy:
                ...
                if caption:
                    replacement = _format_caption(caption)
                    ...
                    if idx >= 0:
                        ...
                        caption_index = idx + len(replacement)
                    else:
    ...
    async def _extract_message_with_caption(
        ...
        for comp in event.get_messages():
            if isinstance(comp, Plain) and comp.text and not voice_transcript:
                ...
                if is_gif:
                    gif_count += 1
                is_platform_sticker = _is_platform_sticker(
                    event, comp, image_component_index
                )
                image_component_index += 1
                # 表情包仍由本插件统一完成图片摘要；Stealer 只负责素材入库，
                # 不再阻断 VLM，否则后续群聊上下文只能看到一个空泛标记。
                if image_url:
